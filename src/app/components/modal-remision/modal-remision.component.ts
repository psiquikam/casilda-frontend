...
export class ModalRemisionComponent implements OnInit {
  ...
  instanciasRemision: MaestroDto[] = [];

  onTipoRemisionChange(): void {
    this.data.cual = '';
    this.instanciasRemision = [];
    if (this.data.tipo?.id) {
      this.listasService.obtenerInstanciasRemision(this.data.tipo.id).subscribe({
        next: (instancias) => {
          this.instanciasRemision = instancias;
        },
        error: () => {
          this.instanciasRemision = [];
        }
